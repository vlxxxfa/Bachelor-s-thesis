...
\subsection{\colorbox{yellow}{Das BASE-Prinzip}}\label{base}

\textbf{BASE} steht für \BAse, \baSe, \basE\ und beschreibt die Alternative zu den strengen \textbf{ACID}-Kriterien.
Bei den Systemen, die nach dem \textbf{BASE}-Prinzip gestaltet sind, wird bewusst in Kauf genommen, dass die Daten nach Schreiboperationen eine gewisse Zeitspanne inkonsistenz sein können. \basE\ besagt, dass die Konsistenz gelockert sein kann.

Bei solchen Systemen ist eher wichtig, dass für alle Clients das System ständig verfügbar ist und nicht, dass die Clients zu dem sofortigen Zeitpunkt die gleichen Daten sehen. Aber nach Ablauf einer gewissen Zeitspanne muss das System den Inkonsistenzen Zustand der Daten wieder in einem konsistenten Zustand bringen, so dass alle Clients die gleichen Daten sehen können\footnote{Das BASE-Prinzip, \url{https://blog.codecentric.de/2011/08/grundlagen-cloud-computing-cap-theorem/}, zugegriffen am 05.02.2017}.
...
\subsubsection{\colorbox{yellow}{Dependency Injection und Mock-Objekte}}\label{di}
