...
Eine Replikatioinsgruppe anzulegen, ist es nur über die sog. \textit{Mongo Shell} auf der Kommandozeile möglich. Um die Replikationsgruppe effizienter erzeugen zu können, wird ein Skript geschrieben, (\textbf{Anhang} \ref{lst:createReplSet}). Das Skript legt neue Ordner an, welche unter Anderem die \textit{logs}-Dateien jeweiliger Server enthalten werden. Des Weiteren wird eine Gruppe erzeugt, Gruppenname definiert und die entsprechenden Server zu der Gruppe zugeordnet.
...
Nach der erfolgreichen Skript-Ausführung veranschaulicht das Ergebnis (Listing \ref{lst:result}), dass eine Replikationsgruppe mit insgesamt 3 Mitgliedern angelegt worden ist.
...
Nachdem die Replikationsgruppe angelegt ist, kann drauf mithilfe des Java \mongo-Treibers zugegriffen werden. Listing \ref{lst:javaZugriff} veranschaulicht das Beispiel.
...
\subsubsection{Rest Modul}
Ein Benutzer hat die Möglichkeit, über verschiedene URLs Fotoalben mit Fotos anzeigen, neue Fotoalben mit neuen Fotos anzulegen, bestehende zu modifizieren oder zu löschen. Die Technologie, in der die Aufgabe umgesetzt warden soll, nähmlich REST, ist lediglich ein Paradigma, mit dessen Hilfe die Features, die das HTTP-Prokoll bietet, möglichst umfassend ausgenutzt warden sollen.
